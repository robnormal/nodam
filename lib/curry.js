...
// we need to use arguments.callee for the makerStack, a debug tool
...
function madeAt(f) {
	if (! f) {
		...
		if (! debugging()) {
			...
			throw new Error('Function has no creationStack! ' +
				'Call creationStack() on the lambda function where it is created.'
			);
		}
	} else {
...
(function() {
	...
	debug = function(bool) {
		...
		if (is_on) {
			// add creationStack functionality to functions that make functions
			_.each(makesFunction, function(f, name) {
				lib[name] = _.compose(creationStack, f);
			});
		} else {
			_.each(makesFunction, function(f, name) {
				lib[name] = f;
