 ...
 *                    passed as first argument to the monadic function
...
function monadize(f, options) {
	return function() {
		var
			args = arguments,
			callback = options.errors ? errFunc : noErrFunc,
			asyncFunc, owner, _stack;

		if (options.method){
			// first argument is owner of method
			owner = args[0];

			asyncFunc = function(mstuff) {
				owner[f].apply(owner,
					__slice.call(args, 1).concat(callback(mstuff)));
			};
		} else {
			asyncFunc = function(mstuff) {
				f.apply(void 0,
					__slice.call(args).concat(callback(mstuff)));
			};
		}
