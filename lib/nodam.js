...
var monadicClassFunctions = {
  ...
  mapM: function(xs, f) {
    var ms = _.map(xs, f);
    if (ms === undefined) {
      throw new R.CheckError('First argument to mapM must be mappable');
    }

    return this.sequence(ms);
  }
};
 ...
 */
var noErrFunc = function(mstuff, options, args, that) {
  return function(u) {
    if (options.maybe) {
      u = M.toMaybe(u);
    }
 ...
 */
var errFunc = function(mstuff, options, args, that) {
...
module.exports = {
  ...
  Either: E,
  restriction: R,
  sqlite: function() { return require('./sqlite.js') },
  _: _
};
