...
var sequenceMe = function(monad, ms) {
	var k = function(m1, m) {
		return m1.pipe(function(xs) {
			return m.pipe(function(x) {
				return monad.result(xs.concat(x));
			});
		});
	};

	return _.reduce(ms, k, monad.result([]));
}

var sequenceMe_ = function(monad, ms) {
	if (!ms.length) return monad.result();
...
var sequence = function(ms) {
	if (! ms.length) throw new Error('Cannot sequence() empty list');

	return sequenceMe(ms[0].constructor, ms);
}

var sequence_ = function(ms) {
	if (! ms.length) throw new Error('Cannot sequence_() empty list');

	return sequenceMe_(ms[0].constructor, ms);
};
...
function fmapM(f, ms) {
	return sequence(_.map(ms, f));
}

function mapM(ms, f) { return fmapM(f, ms); }
...
util.inherits(AsyncFailure, AsyncMonad);
...
_.extend(AsyncFailure.prototype, {
	...
	doBind: function(f) { return this },
		...
		return AsyncMonad.result(this.err).pipe(f);
	},
...
function monadize(f, options) {
	return function() {
		...
		if (debugging()) {
			...
			return new AsyncMonad(asyncFunc);
		}
	};
}
...
module.exports = {
	...
	sequence: sequence,
	sequence_: sequence_,
	sequenceMe: sequenceMe,
	sequenceMe_: sequenceMe_,
	pipeline: pipeline,
	fmapM: fmapM,
	mapM: mapM,
	...
	combineStrict: AsyncMonad.combineStrict,
	get: AsyncMonad.get,
	set: AsyncMonad.set,
