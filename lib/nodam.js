...
// call setFor with whatever's in the monad
AsyncMonad.prototype.setThisFor = function(key, m) {
	var that = this;
	return this.get(key).pipe(function(old) {
		return that.pipe(makeStack(function(x) {
			return that.set(key, x);
		})).then(m).set(key, old);
	});
};
